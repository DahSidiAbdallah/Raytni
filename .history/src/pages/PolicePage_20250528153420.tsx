...
const PolicePage = () => {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      ...
      setLocationError('Geolocation is not supported by this browser.'); // Should also be translated
      setIsLoadingLocation(false);
    }
  }, [t]);
      ...
      <Card className="shadow-lg mb-8">
        ...
        <CardContent className="mt-4">
          ...
          {!isLoadingLocation && !locationError && userLocation && (
             <p className="text-sm text-gray-600 text-center mb-4">{t('page.police.contentText')} {t('showing_nearest_first', { /* Default if key missing */ defaultValue: 'Showing nearest first.' })}</p>
          )}
        </CardContent>
      </Card>
        ...
        {sortedCommissariats.map((commissariat) => (
          <Card key={commissariat.id} className="flex flex-col justify-between shadow-md hover:shadow-lg transition-shadow duration-200">
            ...
            <CardContent className="flex-grow">
              {userLocation && commissariat.distance !== undefined && (
            ...
            <CardFooter>
              <Button 
                ...
                disabled={!userLocation && isLoadingLocation} // Disable if location is still loading
              >
