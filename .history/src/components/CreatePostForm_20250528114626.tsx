...
const CreatePostForm = ({ onBack, onSubmit }: CreatePostFormProps) => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const newMainImageIndex = filesArray.length > 0 ? (formData.mainImageIndex !== null && formData.mainImageIndex < filesArray.length ? formData.mainImageIndex : 0) : null;
              ...
              {imagePreviews.length > 0 && (
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {imagePreviews.map((preview, index) => (
                    <div 
                      key={index} 
                      className={`relative aspect-square border rounded-md overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer ${formData.mainImageIndex === index ? 'border-blue-500 border-2' : 'border-gray-200'}`}
